/**
 * anchor-cloud.js — AnchorBrowser cloud example for IRS e-Services
 *
...
const { login_irs, tin_match } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting IRS e-Services automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_irs(page, {});
      log('Action 1 result:', data1);

      const data2 = await tin_match(page, {});
