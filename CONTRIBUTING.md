# Contributing to IRS e-Services Browser Automation
...
- **Broken selectors**: IRS e-Services updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real IRS e-Services instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.IRS_E_SERVICES_URL}/path/to/section`);
...
When you find working selectors for IRS e-Services, document them:

```javascript
// IRS e-Services selectors (verified YYYY-MM-DD)
...
IRS e-Services updates its UI periodically. When selectors break:
